...
namespace jtl
{
  ...
  static multi_back_insert_group const multi_back_insert_obj{ "multi_back_insert_iterator" };
}

namespace jest
{
  template <> template <>
  void jtl::multi_back_insert_group::test<0>()
  {
    std::string const s{ "testing123" };
    std::string out;
    std::transform(s.begin(), s.end(), jtl::multi_back_inserter(out),
    [](char const c)
    { return "[" + std::string{ c } + "]"; });
  ...
  template <> template <>
  void jtl::multi_back_insert_group::test<1>()
  {
    std::string const s{ "testing123" };
    std::string out;
    std::transform(s.begin(), s.end(), jtl::multi_back_inserter(out),
  ...
  void jtl::multi_back_insert_group::test<2>()
  {
    ...
    std::string out;
    std::copy(v.begin(), v.end(), jtl::multi_back_inserter(out));
    expect_equal(out, "1234");
  }

  template <> template <>
  void jtl::multi_back_insert_group::test<3>()
  {
    std::vector<std::vector<char>> const v{ { '1', '2' }, { '3', '4' } };
